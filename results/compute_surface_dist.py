...
@author: xychen
"""
from metrics import compute_surface_distances, compute_robust_hausdorff, compute_average_surface_distance
...
def hd95(result, reference, voxelspacing=None):
    ...
    _, average_dist = compute_average_surface_distance(surface_distances)
    
    hd_dist_95 = compute_robust_hausdorff(surface_distances, 95)

    return average_dist, hd_dist_95
...
for idx in range(len(subject_list)):
    ...
    avgDist,hausforffDis95 = hd95(label1, groundtruth1, voxelspacing= spacing)
    print('HD | pancreas is: {}'.format(hausforffDis95))
    print('AD | pancreas is: {}'.format(avgDist))
    pancreas.append(hausforffDis95)
    pancreas_asd.append(avgDist)

    print('\n')

print('HD pancreas: ', pancreas)
print(np.mean(pancreas), np.std(pancreas))
print('\n')

print('ASD pancreas: ', pancreas_asd)
print(np.mean(pancreas_asd), np.std(pancreas_asd))
